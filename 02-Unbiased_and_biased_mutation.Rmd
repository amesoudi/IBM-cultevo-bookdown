...
Each generation, there is a probability $\mu$ that each individual mutates from their current trait to the other trait. This probability applies to each individual independently; whether an individual mutates has no bearing on whether or how many other individuals have mutated. On average, that means that $\mu N$ individuals mutate each generation. Like in the previous model, we are interested in tracking the proportion $p$ of agents with trait $A$ over time.

We'll wrap this in a function called `unbiased_mutation()`, using much of the same code as `unbiased_transmission_3()`. Now though we have an extra parameter, $\mu$, to specify the probability of mutation. As before, we need to call the tidyverse library, and set a seed for the random number genrator, so the results will be exactly the same each time we rerun the code.
...
The only changes from the previous model are the addition of $\mu$ in the function definition and new lines of code within the `for` loop on $t$ which replace the random copying command with unbiased mutation. Let's examine these lines to see how they work.
...
To plot the results, we can use the same function `plot_multiple_runs()` we already wrote for the previous model.
