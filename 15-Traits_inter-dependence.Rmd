...
Here the results. To plot them, we use `geom_jitter()`, another ggplot 'geom'. `geom_jitter()` adds a small amount of random variation to the location of each point, regualted by the parameters `width` and `height`: in our case we want a small variation only on the width dimension. This is useful to show data points that would have otherwise being overlapped (techincally defined "overplotting"). We also use the usual `stat_summary()` to plot the mean of the data in red. 
...
The results broadly confirm our intuitions. Interestingly, we do not need all traits being compatible with each other to produce the outcome of all traits being successfull. Already with $\gamma=0.8$, all the 20 traits spread in more than half of the population in almost all runs of the simulation. Even with few incompatiblities, the "compatibility score" between observers and demonstrators is, given a sufficient number of compatible traits, a positive number, resulting in high probabilities to copy.

## Summary of the model
  
Mention the logistic, and the last visualisation

***

## Analytical appendix

I do not know of analytical treatments of this.
