...
Second, we need to keep track of the new introduced traits. In order to do so, we record at the beginning of each generation what is the 'label' of the last trait introduced (at the beginning, with $N=100$, it will be likely to be "100", as we initialise each individual chosing randomly between 1 and 100). When new traits are introduced, we call them with consecutive numbers: the first new traits will be called "101", the second "102" and so on.  
