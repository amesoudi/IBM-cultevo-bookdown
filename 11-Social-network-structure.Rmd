...
This plot shows that the fully connected network reaches saturation faster than the friend network. The difference becomes even more obvious when we re-run this simulation with larger networks:
...
Note, the average network degree is constant at $k =$`r mean(degree(net))`. This is due to the way the networks are created. We start with a regular lattice network and than randomly select an edge and change the vertex at its end (e.g. from A--B to A--C). Thus, while some individuals might now have fewer (or even zero) edges and others might have more, the average number of edges and so the average degree remains fixed an independent of the number of changed edges.
...
This, however, is not true for the path length. Here, we find that larger networks (with otherwise the same network parameters) have longer average path lengths (i.e. it takes on average more steps to get from one to another individual). Also, as we increase the probability to reconnect edges average path lenght goes down. This is because random connections might connect distant sections of the network and so drastically lower the number of steps required to get from one individual to another. 
...
net2 <- watts.strogatz.game(size=11, dim=2, nei=2, p=1)
...
healthBehaviourAdoption <- function(NET, MODE="simple", ROUNDS=100, CONTACTS=1){
  if(NET=="clustered"){
   ...
   net <- watts.strogatz.game(size=11, dim=2, nei=2, p=.5, loops=F, multiple=F)
  }
 adjm <- as.matrix(get.adjacency(net)) 
