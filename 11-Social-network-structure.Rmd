...
ggplot(data, aes(x=groupSize, y=meanTime, col=factor(connectivity))) + geom_line() + geom_point() + theme_bw()
```
...
```{r}
healthBehaviourAdoption <- function(ADJM, MODE="simple", ROUNDS=100){
...
clu_simple <- healthBehaviourAdoption(ADJM=adjm, MODE="simple", ROUNDS=1000)
...
healthBehaviourAdoption <- function(NET, MODE="simple", ROUNDS=100){
  if(NET=="clustered"){
    net <- make_lattice(length=11, dim=2, nei=2, circular=T)
    adjm <- as.matrix(get.adjacency(net))
  } else if(NET=="random"){
  ...
  for(round in 1:ROUNDS){
    ...
    frac <- sum(behaviour[neigh]) / sum(neigh)
    # with probability frequency^2 add behaviour to repertoire
    if(runif(n=1,min=0,max=1) < (frac^mod)){
      behaviour[focal] <- T
    } else {
      # maybe: else have loss of behaviour if frequency^2 is below threshold
      # behaviour[focal] <- F
...
#repeated runs
reps <- 100
clu_simple <- do.call("rbind", lapply(1:reps, function(x) healthBehaviourAdoption(NET="clustered", MODE="simple", ROUNDS=1000)))
clu_comp <- do.call("rbind", lapply(1:reps, function(x) healthBehaviourAdoption(NET="clustered", MODE="complex", ROUNDS=1000)))
clu <- rbind(clu_simple, clu_comp)

net2 <- watts.strogatz.game(size=11, dim=2, nei=2, p=.5)
adjm2 <- as.matrix(get.adjacency(net2))
rand_simple <- do.call("rbind", lapply(1:reps, function(x) healthBehaviourAdoption(NET="random", MODE="simple", ROUNDS=1000)))
rand_comp <- do.call("rbind", lapply(1:reps, function(x) healthBehaviourAdoption(NET="random", MODE="complex", ROUNDS=1000)))
rand <- rbind(rand_simple, rand_comp)
