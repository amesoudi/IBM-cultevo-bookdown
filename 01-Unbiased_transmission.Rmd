...
This first value of $p$ is 0.51, meaning that 51 individuals have trait $A$, and 49 have trait $B$. Even though `sample()` returns either trait with equal probability, this does not necessarily mean that we will get exactly 50 $A$s and 50 $B$s. This happens with simulations and finite population sizes: they are probabilistic (or stochastic), not deterministic. Analogously, flipping a coin 100 times will not always give exactly 50 heads and 50 tails. Sometimes we will get 51 heads, sometimes 49, etc. To see this in our simulation, re-run the above code with different values of the random number seed.
...
```
<!-- This descriptions depend on the random generation. This changes with different R version, so we need to change it -->
...
<!-- AA 24/07/2020 I had to add "as.numeric" when defining the output data structure to make the code 
compile. Otherwise it considered it logical and it was giving an error when assigning to it 
sum(population$trait == "A") / N. This must be something that changed with the more recent version of the tidyverse. This may need to be changed everywhere. -->
