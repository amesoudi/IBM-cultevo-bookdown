...
In our populations, each individual is constantly exhibiting their behaviour. Occasionally, an individual will copy the behaviour they observe by another individual. To simulate this updating, we will add a loop to select a random individual (which is only represented as a behaviour along the `behaviours` vector) and let this individual copy a random other individual:
...
We can plot the frequency of each trait over time, which we recorded in the `recbehav` matrix. :
...
ggplot(recbehav_d) + geom_line(aes(x=time, y=freq/popsize, col=factor(behaviour))) + scale_y_continuous(limits=c(0,1))
```

As we have seen in the chapter on unbiased transmission (REF), the frequency of the two traits will move around 0.5. For smaller populations, more traits, or longer time drift will lead to the exclusion of one of the traits. We can test this by using a much smaller population. 
...
We will keep track which individual belongs to which cluster and then record the frequency of one of the behaviours, $p$, (with two traits the frequency of the other behaviour, $q$, is simply $1-p$) in each cluster. 
...
migrationModel <- function(NPOP, POPSIZE, NBEHAVIOUR, NTIMESTEPS){
  ...
  for(timestep in 1:NTIMESTEPS){
    ...
    # let observer acquire trait from model
    behaviours[ observer_model[1] ] <- behaviours[ observer_model[2] ]
    recbehav[timestep,] <- c(sum( behaviours[cluster==1]==1 ) / sum(cluster==1),
                              sum( behaviours[cluster==2]==1 ) / sum(cluster==2))
  }
...
So far, an individual is equally likely to learn from any other individual in the entire population. What would happen if clusters were completely isolated from each other, and so an individual would only learn from within its cluster? One way to do this is to change the sample function that selects a cultural model. Instead of selecting among all individuals, we limit it to individuals in the same cluster (same cluster ID). In anticipation of migration, let's also add an if statement to check whether there is any model left in that cluster:
