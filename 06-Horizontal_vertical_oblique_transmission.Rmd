...
First we set up an output tibble to store the frequency of $A$ ($p$) over $t_max$ generations and across $r_max$ runs. As before we create a `population` tibble to store our $N$ traits, one per individual.
...
Here we can see a gradual spread of the favoured trait $A$ from $p=0.01$ to $p=1$. As in our directly biased transmission model (Chapter 3), the diffusion curve is s-shaped. To obtain the same result with two different models is encouraging! We can also test our prediction that when $b=0.5$, we recreate our unbiased transmission model from Chapter 1:
